...
function get_field_reference( $field_name, $post_id ) {
	...
	// cache
	$found = false;
	$cache = wp_cache_get( 'field_reference/post_id=' .  $post_id . '/name=' .  $field_name, 'mikos', false, $found );
	if( $found )
	{
		return $cache;
	}
	...
	// set cache
	wp_cache_set( 'field_reference/post_id=' .  $post_id . '/name=' .  $field_name, $return, 'mikos' );
...
//add_shortcode( 'mikos', 'acf_shortcode' );
/*
*  acf_form_head()
*
...
function acf_form( $options = array() )
{
	...
	$defaults = array(
		...
		'html_before_fields' => '',
		'html_after_fields' => '',
		'submit_value' => esc_html__("Update", 'mikos'),
		'updated_message' => esc_html__("Post updated", 'mikos'),
	);
