...
class acf_upgrade
{
	...
	*/
	function admin_menu()
	{
		...
		// update admin page
		//add_submenu_page('edit.php?post_type=acf', __('Upgrade','mikos'), __('Upgrade','mikos'), 'manage_options','acf-upgrade', array($this,'html') );
	}
		...
		switch($_POST['version'])
		{
				...
				if($acfs)
				{
					foreach($acfs as $acf)
					{
						// allorany doesn't need to change!
			 			$rules = $wpdb->get_results( $wpdb->prepare("SELECT * FROM $acf_rules WHERE acf_id = '$acf->ID' ORDER BY order_no ASC", ARRAY_A) );
						if($rules)
						{
			...
			*--------------------*/
			case '3.0.0 (step 3)':
			    ...
			    $parent_id = 0;
			    $fields = $wpdb->get_results( $wpdb->prepare("SELECT * FROM $acf_fields WHERE parent_id = $parent_id ORDER BY order_no, name", ARRAY_A) );
			 	if($fields)
			 	{
					// loop through fields
				 	foreach($fields as $field)
				 	{
						...
						else
						{
				 		...
				 		if($field['type'] == 'repeater')
				 		{
				 			...
				 			$parent_id = $field['id'];
				 			$sub_fields = $wpdb->get_results( $wpdb->prepare("SELECT * FROM $acf_fields WHERE parent_id = $parent_id ORDER BY order_no, name", ARRAY_A) );
			...
			*--------------------*/
			case '3.0.0 (step 4)':
				...
				$values = $wpdb->get_results( $wpdb->prepare("SELECT v.field_id, m.post_id, m.meta_key, m.meta_value FROM $acf_values v LEFT JOIN $wp_postmeta m ON v.value = m.meta_id WHERE v.sub_field_id = 0", ARRAY_A) );
				if($values)
				{
				...
				$values = $wpdb->get_results( $wpdb->prepare("SELECT v.field_id, v.sub_field_id, v.order_no, m.post_id, m.meta_key, m.meta_value FROM $acf_values v LEFT JOIN $wp_postmeta m ON v.value = m.meta_id WHERE v.sub_field_id != 0", ARRAY_A) );
				if($values)
				{
					$rows = array();
					foreach($values as $value)
			...
			*--------------------*/
			case '3.1.8':
				// vars
				$message = printf(esc_attr__("Migrating options values from the %s table to the %s table",'mikos'), $wp_postmeta, $wp_options ) . '...';
				// update normal values
			...
			*--------------------*/
			case '3.2.5':
				// vars
				$message = esc_html__("Modifying field group options 'show on page'",'mikos') . '...';
				// get acf's
				$acfs = get_posts(array(
					'numberposts' 	=> -1,
					'post_type' 	=> 'acf',
			...
			*/
			case '3.3.3':
				// vars
				$message = esc_html__("Modifying field option 'taxonomy'",'mikos') . '...';
				$wp_term_taxonomy = $wpdb->prefix.'term_taxonomy';
				$term_taxonomies = array();
				$rows = $wpdb->get_results( $wpdb->prepare("SELECT * FROM $wp_term_taxonomy", ARRAY_A) );
				if($rows)
				{
					foreach($rows as $row)
					{
			...
			*/
			case '3.4.1':
				// vars
				$message = esc_html__("Moving user custom fields from wp_options to wp_usermeta'",'mikos') . '...';
				$option_row_ids = array();
				$option_rows = $wpdb->get_results( $wpdb->prepare("SELECT option_id, option_name, option_value FROM $wpdb->options WHERE option_name LIKE 'user%' OR option_name LIKE '\_user%'", ARRAY_A) );
				if( $option_rows )
				{
