...
class acf_field_group
{
	...
	*/
	...
	function get_field_groups( $array )
	{
		// cache
		$found = false;
		$cache = wp_cache_get( 'field_groups', 'mikos', false, $found );
	...
	*/
	...
	function admin_head()
	{
		...
		// l10n
		$l10n = array(
			'move_to_trash'		=>	esc_html__("Move to trash. Are you sure?",'mikos'),
			'checked'			=>	esc_html__("checked",'mikos'),
			'no_fields'			=>	esc_html__("No toggle fields available",'mikos'),
			'title'				=>	esc_html__("Field group title is required",'mikos'),
			'copy'				=>	esc_html__("copy",'mikos'),
			'or'				=>	esc_html__("or",'mikos'),
			'fields'			=>	esc_html__("Fields",'mikos'),
			'parent_fields'		=>	esc_html__("Parent fields",'mikos'),
			'sibling_fields'	=>	esc_html__("Sibling fields",'mikos'),
			'hide_show_all'		=>	esc_html__("Hide / Show All",'mikos')
		);
...
(function($) {
	...
	acf.l10n = <?php echo json_encode( $l10n ); ?>;
		...
		// add metaboxes
		add_meta_box('acf_fields', esc_html__("Fields",'mikos'), array($this, 'html_fields'), 'mikos', 'normal', 'high');
		add_meta_box('acf_location', esc_html__("Location",'mikos'), array($this, 'html_location'), 'mikos', 'normal', 'high');
		add_meta_box('acf_options', esc_html__("Options",'mikos'), array($this, 'html_options'), 'mikos', 'normal', 'high');
	...
	*/
	...
	{
	    $current .= '<h5>' . esc_html__("Fields",'mikos') . '</h5>';
	    
	    $current .= '<div class="show-field_key">' . esc_html__("Show Field Key:",'mikos');
			$current .= '<label class="show-field_key-no"><input checked="checked" type="radio" value="0" name="show-field_key" />' . esc_html__("No",'mikos') . '</label>';
			$current .= '<label class="show-field_key-yes"><input type="radio" value="1" name="show-field_key" />' . esc_html__("Yes",'mikos') . '</label>';
		$current .= '</div>';
	...
	function ajax_render_location( $options = array() )
	{
		...
		switch($options['param'])
		{
			...
			case "page_type" :
				...
				$choices = array(
					'front_page'	=>	esc_html__("Front Page",'mikos'),
					'posts_page'	=>	esc_html__("Posts Page",'mikos'),
					'top_level'		=>	esc_html__("Top Level Page (parent of 0)",'mikos'),
					'parent'		=>	esc_html__("Parent Page (has children)",'mikos'),
					'child'			=>	esc_html__("Child Page (has parent)",'mikos'),
				);
			...
			case "page_template" :
				...
				$choices = array(
					'default'	=>	esc_html__("Default Template",'mikos'),
				);
			...
			case "post_status" :
				...
				$choices = array(
					'publish'	=> esc_html__( 'Publish','mikos' ),
					'pending'	=> esc_html__( 'Pending Review','mikos' ),
					'draft'		=> esc_html__( 'Draft','mikos' ),
					'future'	=> esc_html__( 'Future','mikos' ),
					'private'	=> esc_html__( 'Private','mikos' ),
					'inherit'	=> esc_html__( 'Revision','mikos' ),
					'trash'		=> esc_html__( 'Trash','mikos' )
				);
			...
			case "user_type" :
				...
				if( is_multisite() )
				{
					$choices['super_admin'] = esc_html__('Super Admin','mikos');
				}
			...
			case "ef_taxonomy" :
				...
				$choices = array('all' => esc_html__('All', 'mikos'));
			...
			case "ef_user" :
				...
				$choices = array_merge( array('all' => esc_html__('All', 'mikos')), $wp_roles->get_names() );
