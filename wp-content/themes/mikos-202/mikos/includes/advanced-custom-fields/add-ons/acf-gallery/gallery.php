...
class acf_field_gallery extends acf_field
{
	...
	*/
	...
	function __construct()
	{
		// vars
		$this->name = 'gallery';
		$this->label = esc_html__("Gallery",'mikos');
		$this->category = esc_html__("Content",'mikos');
...
acf.fields.gallery.title_add = "<?php esc_html_e("Add Image to Gallery",'mikos'); ?>";
acf.fields.gallery.title_edit = "<?php esc_html_e("Edit Image",'mikos'); ?>";
</script>
<?php
	}
...
<div class="acf-gallery" data-preview_size="<?php echo $field['preview_size']; ?>">
	...
	<div class="thumbnails">
		...
		<?php if( $field['value'] ): foreach( $field['value'] as $attachment ): 
			...
			<div class="thumbnail" data-id="<?php echo $attachment['id']; ?>">
				...
				<div class="inner clearfix">
					...
					<div class="list-data">
						<table>
							<tbody>
							<tr>
								<th><label><?php esc_html_e("Title",'mikos'); ?>:</label></th>
								<td class="td-title"><?php echo $attachment['title']; ?></td>
							</tr>
							<tr>
								<th><label><?php esc_html_e("Alternate Text",'mikos'); ?>:</label></th>
								<td class="td-alt"><?php echo $attachment['alt']; ?></td>
							</tr>
							<tr>
								<th><label><?php esc_html_e("Caption",'mikos'); ?>:</label></th>
								<td class="td-caption"><?php echo $attachment['caption']; ?></td>
							</tr>
							<tr>
								<th><label><?php esc_html_e("Description",'mikos'); ?>:</label></th>
				...
				<div class="hover">
					<ul class="bl">
						<li><a href="#" class="acf-button-delete ir"><?php esc_html_e("Remove",'mikos'); ?></a></li>
						<li><a href="#" class="acf-button-edit ir"><?php esc_html_e("Edit",'mikos'); ?></a></li>
					</ul>
				</div>
	...
	<div class="toolbar">
		<ul class="hl clearfix">
			<li class="add-image-li"><a class="acf-button add-image" href="#"><?php esc_html_e("Add Image",'mikos'); ?></a></li>
			<li class="gallery-li view-grid-li active"><div class="divider divider-left"></div><a class="ir view-grid" href="#"><?php esc_html_e("Grid",'mikos'); ?></a><div class="divider"></div></li>
			<li class="gallery-li view-list-li"><a class="ir view-list" href="#"><?php esc_html_e("List",'mikos'); ?></a><div class="divider"></div></li>
			<li class="gallery-li count-li right">
				<span class="count" data-0="<?php esc_html_e("No images selected",'mikos'); ?>" data-1="<?php esc_html_e("1 image selected",'mikos'); ?>" data-2="<?php esc_html_e("{count} images selected",'mikos'); ?>"></span>
			</li>
		</ul>
	</div>
...
<tr class="field_option field_option_<?php echo $this->name; ?>">
	<td class="label">
		<label><?php esc_html_e("Preview Size",'mikos'); ?></label>
		<p class="description"><?php esc_html_e("Thumbnail is advised",'mikos'); ?></p>
	</td>
	<td>
		<?php
...
(function($){
	...
	// add message
	self.parent.acf.helpers.add_message('<?php esc_html_e("Image Updated",'mikos'); ?>.', div);
...
<?php function mikos_gallery_styles() {
	$styles = '';
	$styles .= '#media-upload-header #sidemenu li#tab-type_url,'
	...
	. '#media-items .media-item .slidetoggle {display: none !important;}'
	. '#media-items .media-item {position: relative; overflow: hidden;}'
	. '#media-items .media-item .acf-checkbox {float: left; margin: 28px 10px 0;}'
	. '#media-items .media-item .pinkynail {max-width: 64px;max-height: 64px;display: block !important;margin: 2px;}'
	. '#media-items .media-item .filename.new {min-height: 0;padding: 20px 10px 10px 10px;line-height: 15px;}'
	. '#media-items .media-item .title {line-height: 14px;}'
	. '#media-items .media-item .acf-select {float: right;margin: 22px 12px 0 10px;}'
	. '#media-upload .ml-submit {display: none !important;}'
	. '#media-upload .acf-submit {margin: 1em 0;padding: 1em 0;position: relative;overflow: hidden;display: none; /* default is hidden */clear: both;}'
	. '#media-upload .acf-submit a {float: left;margin: 0 10px 0 0;}'
	. '.acf-message-wrapper .message {margin: 2px 2px 0;}'
	. '#media-items .media-item.media-item-added {background: #F9F9F9;}'
	. '#wpcontent {margin-left: 0 !important;}';

	if( $options['tab'] == 'gallery' ) {
		$styles .= '#sort-buttons,'
...
add_action('wp_enqueue_scripts', 'mikos_gallery_styles'); ?>
...
(function($){
	...
	*/
	...
	function disable_image( div )
	{
		...
		// change button text
		div.find('.acf-select').addClass('disabled').text("<?php esc_html_e("Added",'mikos'); ?>");
	...
	*/
	...
	$('#media-items .media-item a.acf-select').live('click', function(){
		...
		{
			alert("<?php esc_html_e("Image already exists in gallery",'mikos'); ?>");
			return false;
		}
	...
	function acf_add_buttons()
	{
		...
		$('#media-items .media-item:not(.acf-active)').each(function(){
			...
			// Add select button
			$(this).find('.filename.new').before('<a href="' + id + '" class="button acf-select"><?php esc_html_e("Add Image",'mikos'); ?></a>');
			
			// add save changes button
			$(this).find('tr.submit input.button').hide().before('<input type="submit" value="<?php esc_html_e("Update Image",'mikos'); ?>" class="button savebutton" />');
