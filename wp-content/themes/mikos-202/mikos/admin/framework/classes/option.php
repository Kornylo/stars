...
class VP_Option
{
	...
	public function __construct(array $configs)
	{
		...
		$configs = array_merge(array(
			...
			'layout'                => 'fixed',
			'page_title'            => esc_html__( 'Vafpress Options', 'mikos' ),
			'menu_label'            => esc_html__( 'Vafpress Options', 'mikos' ),
			'priority'              => 10,
		), $configs);
		...
		else throw new Exception(esc_html__( 'Option Key is required', 'mikos' ), 1);
		if(isset($template)) $this->set_template($template);
		else throw new Exception(esc_html__( 'Template Array/File is required', 'mikos' ), 1);
		if(isset($page_slug)) $this->set_page_slug($page_slug);
		else throw new Exception(esc_html__( 'Page Slug is required', 'mikos' ), 1);
		// swim in the pool
	...
	{
		VP_WP_Util::admin_notice(__("Development Mode is Active, options' values won't be saved into database.", 'mikos'), false);
	}
	...
	function ajax_import_option()
	{
		...
		if($result['status'])
		{
			...
			{
				$result['status']  = false;
				$result['message'] = esc_html__("Can not be empty", 'mikos');
			}
			else
			{
				...
				{
					$result['status']  = false;
					$result['message'] = esc_html__("Invalid data", 'mikos');
				}
			}
		}
	...
	function ajax_export_option()
	{
		...
		if($result['status'])
		{
			...
			$result = array(
				'status' => true,
				'message'=> esc_html__("Successful", 'mikos'),
				'option' => $sr_options,
			);
		}
	...
	function vp_verify_nonce()
	{
		...
		{
			$result['status']  = true;
			$result['message'] = esc_html__("Successful", 'mikos');
		}
		else
		{
			$result['status']  = false;
			$result['message'] = esc_html__("Unverified Access", 'mikos');
		}
		return $result;
	}
	...
	public function init_options_set()
	{
		...
		if( is_string($this->get_template()) and is_file($this->get_template()) )
			...
			throw new Exception(__( 'Invalid template supplied', 'mikos' ), 1);
		...
		if( $this->use_util_menu() )
		{
			...
			$util_menu->set_title(__('Utility', 'mikos'));
			...
			$restore_section->set_title(__('Restore Default', 'mikos'));
			...
			$exim_section->set_title(__('Export/Import', 'mikos'));
